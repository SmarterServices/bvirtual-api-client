...
class MatrixFormatter {
   ...
   */
  format() {
    let tempFormattedMatrix = {};
    let datesArray = [];
    for (let timeSlotsOfDay of this.matrix) {
      let date = this.getDate(timeSlotsOfDay);
      // keep the formatted object key in the array
      datesArray.push(date);
      // keep the schedules on a temp object
      tempFormattedMatrix[date] = this.formatTimeSlot(timeSlotsOfDay);
    }
    ...
    for(let date of datesArray) {
      this.formattedMatrix[date] = tempFormattedMatrix[date];
    }
