...
const timeSlots = [
  {
    startTime: moment(`${currentYear}-01-01T00:00:00.000Z`).utc(),
    endTime: moment(`${currentYear}-01-01T06:00:00.000Z`).utc()
  },
  {
    startTime: moment(`${currentYear}-01-01T06:00:00.000Z`).utc(),
    endTime: moment(`${currentYear}-01-01T12:00:00.000Z`).utc()
  },
  {
    startTime: moment(`${currentYear}-01-01T12:00:00.000Z`).utc(),
    endTime: moment(`${currentYear}-01-01T18:00:00.000Z`).utc()
  },
  {
    startTime: moment(`${currentYear}-01-01T18:00:00.000Z`).utc(),
    endTime: moment(`${currentYear}-01-01T23:59:59.000Z`).utc()
  }
];
...
function getAvailableSlotMatrix(payload, bVirtualClient) {
  // make selection default as 'true'
  let useSelectionLogic = !(payload.selection === false);
  ...
  return getSchedulesPerDay(payload)
    .then(function (schedules) {
      ...
      if(useSelectionLogic) {
        // build the original matrix using selection logic
        matrix = buildMatrix(unsatisfiedMatrix);
      } else {
        matrix = unsatisfiedMatrix;
      }
...
function getSchedulesOfDay(fromTime, toTime, inclusion, data) {
  ...
  var _start = moment(parseInt(payload.startDateLong));
  var _end = moment(parseInt(payload.endDateLong));

  payload.startDateLong = _start.startOf('day').valueOf();
  payload.endDateLong = _end.endOf('day').add(1, 'ms').valueOf()
