...
function getAvailableSlotMatrix(payload, bVirtualClient) {
  ...
  return getSchedulesPerDay(payload)
    .then(function (schedules) {
      ...
      // add 'localStart' filed with localTime
      let schedulesWithLocalTime = MatrixFormatter.addLocalTime(schedules, payload.tzOffset);
      // update schedule array according to 'locaStart' time
      schedulesWithLocalTime = MatrixFormatter.categorizeWithLocalTime(schedulesWithLocalTime);
 ...
 * @rejects {Error}
 */
function getSchedulesOfDay(date, inclusion, data) {
...
function getSchedulesPerDay(data) {
  ...
  for (let i = 1; i <= totalDates; ++i) {
    ...
    if (i === 1) {
      ...
      schedulePromise = getSchedulesOfDay(startDate, '(]', data);
    } else if (i === totalDates) {
      // get schedules for last day from beginning of day till calculated time
      schedulePromise = getSchedulesOfDay(moment(adjustedEndDate), '[)', data);
    } else {
...
function structureSchedule(dates) {
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    let scheduleDate = moment(schedule.localStart);
...
function limitSchedulesInSlot(schedulesOfDay, slotNumber, itemTakenPerSlot) {
  ...
  while (scheduleTaken < itemsNeedInThisSlot) {
    ...
    schedules.splice(randomScheduleIndex, 0);
