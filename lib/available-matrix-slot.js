...
function takeDay(matrix, schedulesOfDay, itemTakenPerSlot) {
  ...
  for (let i = 0; i < itemTakenPerSlot.length; ++i) {
    ...
    schedulesInSlot = limitSchedulesInSlot(schedulesOfDay[i]);
 ...
 * @param {Array<Object>} schedules - A time slot containing all the schedules
 * @returns {Array<Object>} - Minimum number of schedules for this time slot
 */
function limitSchedulesInSlot(schedules) {
  let randomScheduleIndex = undefined;
  let randomSchedule = undefined;
  let limitedSchedules = [];
  let scheduleTaken = 0;

  if(schedules.length <= minItemsRequiredPerSlot) {
    return schedules;
  }

  while (scheduleTaken < minItemsRequiredPerSlot) {
